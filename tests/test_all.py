...
def test_batteria_scritta():
    """La batteria composta a mano: i vincoli, non le regole.

    ⚠️ Non controlla che segua una formula -- non ne segue nessuna, ed e' il
    punto. Controlla i VINCOLI dichiarati in `docs/istruzioni/batteria-jazz.md`
    e i difetti che hanno fatto respingere le versioni 13, 14 e 15.
    """
    import batteria_scritta as BS                           # noqa: PLC0415
    import genera_jazz as GJ                                # noqa: PLC0415

    battute = len(GJ.BLUES.giro) * GJ.GIRI
    parte = BS.per_battuta(battute)
    vuoto = '.' * 16

    check('la parte ha le battute del pezzo', len(parte) == battute,
          str(len(parte)))
    check('il ride non si buca mai',
          all(p['ride'] == BS.COSTANTI['ride'] for p in parte))
    check('il charleston sta su 2 e 4 in ogni battuta',
          all(p['charleston a pedale'] == BS.COSTANTI['charleston a pedale']
              for p in parte))

    # il difetto della 13: voci mute per intere battute
    check('la cassa c e in ogni battuta',
          all('kick' in p for p in parte))
    # il difetto della 15: il rullante che raddoppia il charleston sempre
    doppia = sum(1 for p in parte if p.get('rullante', vuoto)[4] == 'x')
    check('il rullante non raddoppia il charleston in ogni battuta',
          doppia <= battute // 3, f'{doppia} battute su {battute}')
    # e il difetto della 14: riempire tutto
    muti = sum(1 for p in parte if 'rullante' not in p)
    check('il rullante tace in parecchie battute', muti >= 6,
          f'{muti} su {battute}')

    # la cassa e leggera: feathering, non quattro movimenti
    pesanti = [i + 1 for i, p in enumerate(parte) if p['kick'].count('x') > 3]
    check('la cassa resta leggera, mai piu di tre colpi', not pesanti,
          str(pesanti))

    colpi = [sum(p.get(v, vuoto).count('x') for v in
                 ('ride', 'charleston a pedale', 'rullante', 'kick'))
             for p in parte]
    check('i colpi per battuta stanno fra otto e sedici',
          all(8 <= c <= 16 for c in colpi), f'min {min(colpi)}, max {max(colpi)}')

    # le righe malformate devono fermare tutto
    for cattiva, perche in (
            ('   1   ...........  x.......x.......', 'pattern corto'),
            ('   2   ................  x.......x.......', 'battuta fuori ordine'),
            ('   1   ....y...........  x.......x.......', 'carattere non valido')):
        try:
            BS.leggi(cattiva)
