...
| **lunghezza della clip in movimenti** | `S.set_clip_length_beats(doc.root, clip, 3)` imposta 3 movimenti (3/4); `3.5` dà 7/8. I tick seguono `inputTickMagnitude` della song. Il Deluge non salva un metro globale: ogni clip ha la propria lunghezza. La funzione restituisce gli avvisi di `S.set_clip_length` per le note rimaste oltre la fine |
| **figure su una risoluzione diversa** | Calcolare `barra = S.ticks_per_bar(doc.root)` e passare `tick_per_battuta=barra` a `MU.passi`, `MU.durata_in_tick`, `MU.melodia`, `MU.linea`, `MU.accordi`, `MU.armonia`, `MU.comping` e `MU.reazione`. Il default di 384 tick vale solo per song con `inputTickMagnitude="2"`; `MU.passi(..., lunghezza=...)` continua a ricevere una durata grezza in tick. La lunghezza predefinita delle nuove clip synth, audio, MIDI e CV segue invece automaticamente la song. |
| **lunghezza indipendente di riga** | `MU.lunghezza_riga(doc, clip, dove, passi, durata='1/16')` — `dove` è il drum o l'altezza, `passi` conta le figure sulla griglia reale della song. `None` fa tornare la riga a seguire la clip. Le altre righe e la lunghezza principale non cambiano |
...
in loco** in `~/.Codex/skills/music-composer/scripts/`. È una
